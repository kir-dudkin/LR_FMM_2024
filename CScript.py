...
def write_KIR_FILES(project_name, target_dir=TARGET_PATH,rewrite_input=False):
...
def copy_input_files( project_name, target_dir=TARGET_PATH):
    copyfile(SOURCE_PATH, target_dir, project_name)
    copyfile(SOURCE_PATH, target_dir, "physics.dat")
    return 0

def run_kir(target_dir=TARGET_PATH, check_code=False):
    if check_code:
